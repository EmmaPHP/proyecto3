<?php
class UnionGastosCatModel extends Model{
    private $expenseId;
    private $titulo;
    private $amount;
    private $categoryId;
    ...
    public function getAll($userid){
        $items = [];
        try{
            $query = $this->prepare('SELECT expenses.id as expense_id, title, category_id, amount, 
            date, id_user, categories.id, name, color  FROM expenses INNER JOIN categories 
            WHERE expenses.category_id = categories.id AND expenses.id_user = :userid ORDER BY date');
            $query->execute(["userid" => $userid]);
            while($p = $query->fetch(PDO::FETCH_ASSOC)){
                $item = new UnionGastosCatModel();
                $item->from($p);
    ...
    function getTotalByMonthAndCategory($date, $categoryid, $userid){
        try{
            ...
            WHERE category_id = :val AND id_user = :user AND YEAR(date) = :year 
            AND MONTH(date) = :month');
            $query->execute(['val' => $categoryid, 'user' => $userid, 'year' => $year, 
            'month' => $month]);
            if($query->rowCount() > 0){
